...
`hold` 用于保存一条值得长期保留的事实、经历、偏好或承诺，`comment_bucket` 用于给已有记忆增加年轮。

`grow` 不再是长文脱水或自动写入入口。它只在一个聊天窗口结束时，保存当前 AI 亲自写下的完整第一人称“窗影”。整篇原文进入独立 `window_shadows.sqlite`，不参与普通召回；第五层里显式写成 `### moment` 的场景会原样复制为普通 bucket。旧 `grow(auto=true)` / Operit 摘记路径已停用，因此当前 `grow` 不再需要写入门卫。

画像维护会读取最近几篇连续窗影，只把它们用于 persona / relationship；单篇只能影响 recent / staging / mid-term，至少两篇独立窗影重复出现的变化才允许支持 stable。新窗口仍以画像为主，只在最新窗影尚未被画像模型读过时临时带最近一篇。
