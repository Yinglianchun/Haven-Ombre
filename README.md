...
普通 `hold` 的 `### scene` 是当前 AI 写下的场景真源，不是脱水摘要。新写入只允许这一段；旧 body / `moment` / `original` / `reflection` 仍兼容读取，但不再作为新写入 section。旧 `affect_anchor` 也只兼容读取。
...
`hold` 只原样保存一件由当前 AI 写好的长期 Scene：content 必须且只能有一段 `### scene`，重要原话和意义直接写在场景里；工具不调用脱水/标签模型，也不合并旧桶。`cues` 可在同一次调用中写入 0～8 个 sidecar 召回入口。稳定偏好、边界和身份事实先有证据 Scene，再用 `profile_fact` 建立索引。
...
`profile_fact` 是可搜索的事实目录，不是普通记忆：它不进入普通候选、扩散或 handoff。显式事实问题命中 Fact 后，系统沿 evidence 返回 canonical Scene。后台模型生成的 tags / cues / importance / confidence 只写入 `metadata_proposals.sqlite`；独立 Scene linker 的关系提案写入 `scene_edge_proposals.sqlite`。提案可通过 `scene_edge_proposals` 只读查看；只有逐条显式确认的 `review_scene_edge_proposal` 会在重新校验两端 Scene hash、active 状态与逐字证据后把一条边写入正式图。没有批量自动转正。
